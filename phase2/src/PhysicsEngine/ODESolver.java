...
public class ODESolver implements ODESolverInterface {
    ...
    public StateInterface[] solve(ODEFunctionInterface f, StateInterface y0, double tf, double h) {
        ...
        if(DEBUG){
            System.out.println("PhysicsEngine.PhysicsEngine.Planets.Solvers.ODESolver - states " + states.length);
            System.out.println("PhysicsEngine.PhysicsEngine.Planets.Solvers.ODESolver - state at 0\n" + y0.toString());
        }
        ...
        if(DEBUG){
            for(int i = 0; i < (states.length); i++){
                if(i % 10 == 0){ //print every day
                    System.out.println("PhysicsEngine.PhysicsEngine.Planets.Solvers.ODESolver - state at " + (i) + "\n" + states[i].toString());
                }
            }
        }
        return states;
